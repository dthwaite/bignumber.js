    ...
    BigNumber.prototype.multiply = function(number) {
        ...
        for (var index = 0; index < this.number.length; index++) {
            for (var remainder = 0, givenNumberIndex = 0; givenNumberIndex < bigNumber.number.length; givenNumberIndex++) {
                var temp= remainder + this.number[index] * bigNumber.number[givenNumberIndex]+(result[index + givenNumberIndex] || 0);
                result[index + givenNumberIndex]= temp % base;
                remainder = Math.floor(temp / base);
            }
        ...
        case 1:
            ...
            if (denominator.length==1) {
                ...
                for (index = this.number.length - 1; index >= 0; index--) {
                    rest.unshift(this.number[index]); // Stacks the numerator digit as the LSB
                    ...
                    rest.pop(); // Discards now redundant zero MSB
                }
                this.rest = BigNumber();
