...
!(function() {
    ...
    // Very efficient comarison of two BigNumbers, returns true if a<=b (assumes a is normally larger)
    BigNumber._lte=function(a,b) {
        if (a.number.length> b.number.length) return false;
        if (a.number.length== b.number.length) {
            for (var index = a.number.length - 1; index >= 0; index--) {
    ...
    BigNumber._simplemultiply=function(a,b,remainder) {
        for (var index = 0; index < a.number.length; index++) {
            var temp=remainder + a.number[index] * b;
            a.number[index] = temp % base;
            remainder = Math.floor(temp / base);
        }
        if (remainder) a.number.push(remainder);
        return a.number;
    };
    ...
    BigNumber.prototype.divide = function(number) {
        ...
        for (index = this.number.length - 1; index >= 0; index--) {
            if (nativebigNumber) {
                ...
                var temp=nativerest*base+this.number[index];
                result[index]=Math.floor(temp/nativebigNumber);
                nativerest=temp%nativebigNumber;
                BigNumber.stats.divs.iterations++;
            } else {
                // Otherwise we need to use BigNumber arithmetic - but the very efficient ones
                ...
                for (var i=0;i<factors.length;i++) {
                    divisor/=factors[i];
                    BigNumber._simplemultiply(rest,factors[i],Math.floor(digit/divisor));
                    digit=digit%divisor;
                    while (BigNumber._lte(bigNumber,rest)) {
                        result[index]+=divisor;
                        BigNumber._subtract(rest,bigNumber);
                        BigNumber.stats.divs.iterations++;
                    }
                }
            }
    ...
    BigNumber.prototype.sqrt=function() {
        ...
        do {
            ...
            BigNumber.stats.sqrt.iterations++;
        } while (!last.equals(next));
        this.number=last.number.slice();
        BigNumber.stats.sqrt.count++;
        return this;
    };
